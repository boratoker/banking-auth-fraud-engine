...
# Normal işlem özellikleri (Gürültü artırıldı)
normal_data = {
    'amount': np.random.lognormal(mean=7, sigma=1.5, size=N_NORMAL).clip(5, 80000),
    'amount_to_user_avg_ratio': np.random.uniform(0.1, 4.5, N_NORMAL),
    'currency_mismatch': np.random.binomial(1, 0.05, N_NORMAL),
    'is_fast_type': np.random.binomial(1, 0.6, N_NORMAL),
    'tx_count_last_1h': np.random.poisson(1.0, N_NORMAL),
    'tx_amount_sum_last_24h': np.random.lognormal(mean=7.5, sigma=1.2, size=N_NORMAL).clip(0, 150000),
    'daily_limit_usage_pct': np.random.uniform(0.0, 0.8, N_NORMAL),
    'time_since_last_tx_sec': np.random.exponential(3600, N_NORMAL).clip(10, 86400),
    'is_night_time': np.random.binomial(1, 0.15, N_NORMAL),
    'is_new_recipient_iban': np.random.binomial(1, 0.25, N_NORMAL),
    'recipient_transfers_count': np.random.poisson(4, N_NORMAL),
    'recipient_in_contacts': np.random.binomial(1, 0.6, N_NORMAL),
    'device_fingerprint_matched': np.random.binomial(1, 0.85, N_NORMAL),
    'ip_location_distance_km': np.random.exponential(15, N_NORMAL).clip(0, 300),
    'recent_failed_logins': np.random.binomial(1, 0.08, N_NORMAL),
    'session_age_minutes': np.random.uniform(2, 480, N_NORMAL),
    'amount_is_round': np.random.binomial(1, 0.25, N_NORMAL),
...
# Fraud işlem özellikleri (Gerçekçi, daha fazla çakışan kalıplar)
fraud_data = {
    'amount': np.random.lognormal(mean=8.5, sigma=1.2, size=N_FRAUD).clip(1000, 150000),
    'amount_to_user_avg_ratio': np.random.uniform(1.5, 15.0, N_FRAUD),
    'currency_mismatch': np.random.binomial(1, 0.15, N_FRAUD),
    'is_fast_type': np.random.binomial(1, 0.8, N_FRAUD),
    'tx_count_last_1h': np.random.poisson(2.5, N_FRAUD),
    'tx_amount_sum_last_24h': np.random.lognormal(mean=8.5, sigma=1, size=N_FRAUD).clip(2000, 300000),
    'daily_limit_usage_pct': np.random.uniform(0.4, 1.0, N_FRAUD),
    'time_since_last_tx_sec': np.random.exponential(300, N_FRAUD).clip(5, 3600),
    'is_night_time': np.random.binomial(1, 0.35, N_FRAUD),
    'is_new_recipient_iban': np.random.binomial(1, 0.65, N_FRAUD),
    'recipient_transfers_count': np.random.poisson(1, N_FRAUD),
    'recipient_in_contacts': np.random.binomial(1, 0.2, N_FRAUD),
    'device_fingerprint_matched': np.random.binomial(1, 0.45, N_FRAUD),
    'ip_location_distance_km': np.random.exponential(80, N_FRAUD).clip(10, 2000),
    'recent_failed_logins': np.random.binomial(3, 0.25, N_FRAUD),
    'session_age_minutes': np.random.uniform(0, 30, N_FRAUD),
    'amount_is_round': np.random.binomial(1, 0.45, N_FRAUD),
